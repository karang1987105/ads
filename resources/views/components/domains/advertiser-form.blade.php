<x-form name="add" method="POST">
    @isset($domain)
        <x-slot name="action">
            {{ route('admin.advertisers.domains.update', compact('domain'), false) }}
        </x-slot>
        <x-slot name="method">PUT</x-slot>
        <x-slot name="submit">Ads.item.submitForm(this)</x-slot>
    @else
        <x-slot name="action">
            {{ route('admin.advertisers.domains.store', ['advertiser' => $advertiser], false) }}
        </x-slot>
